...
GameApp::GameApp(HINSTANCE instance, bool test)
    : D3DApp(instance, L"41 | Godot Water Comparison", 1440, 900), validation(test)
{
}
bool GameApp::Init()
{
    ...
    dynamic.Init(m_pd3dDevice.Get(), m_pd3dImmediateContext.Get(), false);
    compute.Init(m_pd3dDevice.Get(), m_pd3dImmediateContext.Get(), true);
...
void GameApp::Advance(float dt)
{
    ...
    while (accumulator >= 1.f / 60)
    {
        accumulator -= 1.f / 60;
        time += 1.f / 60;
        left.a = powf(waveSpeed * left.n / 60.f, 2);
        left.land = landEnabled;
        float angle = time * .55f;
        float bx = .64f + .19f * cosf(angle), by = .49f + .23f * sinf(angle);
        float heading = atan2f(.23f * cosf(angle), -.19f * sinf(angle));
        left.boat = {bx, by, heading, boatEnabled ? .8f : 0};
        left.drop = {0, 0, 0, 0};
        right.drop = {0, 0, 0, 0};
        if (pendingLeft.x >= 0)
        {
            left.drop = {pendingLeft.x * left.n, pendingLeft.y * left.n, .8f, 0};
            pendingLeft = {-1, -1};
        }
        if (pendingRight.x >= 0)
        {
            right.drop = {pendingRight.x * right.n, pendingRight.y * right.n, 5, 0};
            pendingRight = {-1, -1};
        }
        rainTimer += 1.f / 60;
        if (rain && rainTimer >= .1f && right.drop.z == 0)
        {
            rainTimer = 0;
            random = 1664525 * random + 1013904223;
            float x = float(random % right.n);
            random = 1664525 * random + 1013904223;
            right.drop = {x, float(random % right.n), 3, 0};
        }
        dynamic.Step(left);
        compute.Step(right);
    }
...
void GameApp::UpdateScene(float dt)
{
    ...
    ImGui::Text("41  /  GODOT WATER COMPARISON");
    ImGui::SameLine();
    ImGui::TextDisabled("Pixel shader vs Compute shader | fixed 60 Hz");
    ...
    if (ImGui::Combo("Resolution", &quality,
                     "128\0"
                     "256\0"
                     "512\0"))
        Reset();
    ...
    ImGui::Combo("View", &layout,
                 "Side by side\0"
                 "Dynamic water\0"
                 "Compute texture\0");
    ImGui::SameLine();
    ImGui::SetNextItemWidth(130);
    ImGui::Combo("Display", &display,
                 "Shaded water\0"
                 "Height map\0"
                 "Hull mask\0");
    if (ImGui::Button("Reset left"))
        dynamic.Reset(left.n);
    ImGui::SameLine();
    if (ImGui::Button("Reset right"))
        compute.Reset(right.n);
    ImGui::SameLine();
    ImGui::Checkbox("Moving hull", &boatEnabled);
    ImGui::SameLine();
    ImGui::Checkbox("Island mask", &landEnabled);
    ImGui::SameLine();
    ImGui::Checkbox("Rain drops", &rain);
    ImGui::SetNextItemWidth(150);
    ImGui::SliderFloat("Wave speed", &waveSpeed, .01f, .08f, "%.3f");
    ImGui::SameLine();
    ImGui::SetNextItemWidth(150);
    ImGui::SliderFloat("Wake strength", &left.amplitude, .05f, 1);
    ImGui::SameLine();
    ImGui::SetNextItemWidth(150);
    ImGui::SliderFloat("Compute damping", &right.damp, 1, 10);
    ImGui::SameLine();
    ImGui::TextDisabled("%.1f fps", io.Framerate);
    ImGui::End();
    ImGui::SetNextWindowPos(ImVec2(24, 174));
    ImGui::SetNextWindowBgAlpha(.7f);
    ImGui::Begin("Legend", nullptr,
                 ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoInputs);
    if (layout == 2)
    {
        ImGui::TextColored(ImVec4(.45f, .77f, 1, 1), "COMPUTE TEXTURE / rain + pointer ripples");
        ImGui::Text("R32F | 8 x 8 groups | normal-map surface");
    }
    else
    {
        ImGui::TextColored(ImVec4(.3f, 1, .72f, 1), "DYNAMIC WATER / R - G + hull entry / exit");
        ImGui::Text("RGBA8 | fragment pass | island + displacement");
    }
    ImGui::End();
    if (layout == 0)
    {
        ImGui::SetNextWindowPos(ImVec2(float(m_ClientWidth) / 2 + 24, 174));
        ImGui::SetNextWindowBgAlpha(.7f);
        ImGui::Begin("Compute legend", nullptr,
                     ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoInputs);
        ImGui::TextColored(ImVec4(.45f, .77f, 1, 1), "COMPUTE TEXTURE  /  rain + pointer ripples");
        ImGui::Text("R32F | 8 x 8 groups | normal-map surface");
        ImGui::End();
    ...
    ImGui::Begin("Help", nullptr,
                 ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoInputs);
    ImGui::Text("LMB: disturb water    RMB: orbit    Wheel: zoom    Shaders compile from local HLSL");
    ImGui::End();
    if (!io.WantCaptureMouse)
    {
        ...
        {
            float width = float(m_ClientWidth) / (layout == 0 ? 2 : 1);
            int pane = layout == 0 ? int(io.MousePos.x / width) : 0;
            int mode = layout == 0 ? pane : layout - 1;
            XMMATRIX proj = XMMatrixPerspectiveFovLH(.85f, width / (m_ClientHeight - 165), .1f, 150);
            auto a = XMVector3Unproject(XMVectorSet(io.MousePos.x, io.MousePos.y, 0, 1), pane * width, 165, width,
                                        float(m_ClientHeight - 165), 0, 1, proj, CameraView(), XMMatrixIdentity());
            auto b = XMVector3Unproject(XMVectorSet(io.MousePos.x, io.MousePos.y, 1, 1), pane * width, 165, width,
                                        float(m_ClientHeight - 165), 0, 1, proj, CameraView(), XMMatrixIdentity());
            XMFLOAT3 from, dir;
            ...
            if (fabsf(dir.y) > 1e-6f)
            {
                ...
                if (t > 0 && uv.x >= 0 && uv.x < 1 && uv.y >= 0 && uv.y < 1)
                    (mode == 0 ? pendingLeft : pendingRight) = uv;
            }
        }
    }
    Advance(dt);
...
void GameApp::RenderWater(WaterSimulation &water, int mode, const D3D11_VIEWPORT &vp)
{
    ...
    s.info = {float(mode), float(water.Size()), float(display), heightScale};
    s.boat = left.boat;
    ID3D11ShaderResourceView *srv[] = {water.Height(), water.Collision(), water.Land()};
    c->VSSetShaderResources(0, 3, srv);
    ...
    {
        if (landEnabled)
            draw({-3.96f, 0, 6.08f, 2});
        if (boatEnabled)
            draw({(left.boat.x - .5f) * 22, .18f, (left.boat.y - .5f) * 22, 3});
    }
}
void GameApp::DrawScene()
{
    ...
    float width = float(m_ClientWidth) / (layout == 0 ? 2 : 1);
    if (layout != 2)
        RenderWater(dynamic, 0, {0, 165, width, float(m_ClientHeight - 165), 0, 1});
    if (layout != 1)
        RenderWater(compute, 1, {layout == 0 ? width : 0, 165, width, float(m_ClientHeight - 165), 0, 1});
    ImGui_ImplDX11_RenderDrawData(ImGui::GetDrawData());
    if (validation && frames == 239)
        Capture(L"validation-comparison.png");
...
void GameApp::NumericalTests(std::ofstream &out)
{
    ...
    for (bool cs : {false, true})
    {
        ...
        test.Init(m_pd3dDevice.Get(), m_pd3dImmediateContext.Get(), cs);
        test.Reset(19);
        SimParams p;
        p.n = 19;
        p.a = .2f;
        ...
        for (int step = 0; step < 12; step++)
        {
            ...
            for (int y = 0; y < 19; y++)
                for (int x = 0; x < 19; x++)
                {
                    ...
                    float v =
                        2 * cur[i] - old[i] +
                        (cs ? .25f : p.a) * (at(x - 1, y) + at(x + 1, y) + at(x, y - 1) + at(x, y + 1) - 4 * cur[i]);
                    if (cs)
                        v *= 1 - p.damp * .001f;
                    if (step == 0 && x == 9 && y == 9)
                        v = p.drop.z;
    ...
    // A departing waterline must create the negative (green) component.
    WaterSimulation hullTest;
    hullTest.Init(m_pd3dDevice.Get(), m_pd3dImmediateContext.Get(), false);
    hullTest.Reset(128);
    SimParams hp;
    hp.n = 128;
    hp.land = 0;
    ...
    hp.boat = {.5f, .5f, 0, .8f};
    hullTest.Step(hp);
    auto entered = hullTest.Readback();
    if (entered[64 * 128 + 64].x < .39f)
        throw std::runtime_error("Hull entry did not inject red wave");
    hp.boat.x = .75f;
    hullTest.Step(hp);
    auto departed = hullTest.Readback();
    if (departed[64 * 128 + 64].y < .39f)
        throw std::runtime_error("Hull exit did not inject green wave");
    out << "PASS hull entry/exit positive and negative waves\n";
}
int GameApp::Validate()
{
    ...
    for (auto *water : {&dynamic, &compute})
    {
        ...
        out << "Live " << (water == &dynamic ? "dynamic" : "compute") << ": active=" << active << " Rmax=" << maxPos
            << " Gmax=" << maxNeg << "\n";
        if (active < 20 || maxPos <= 0 || (water == &dynamic && maxNeg <= 0))
            throw std::runtime_error("Missing live wave propagation");
    }
    // Exercise both view modes, both data views, pause, pointer injection and window resize.
    ...
    display = 0;
    paused = true;
    auto before = compute.Readback();
    Advance(.05f);
    auto after = compute.Readback();
    ...
    pendingRight = {.5f, .5f};
    Advance(1.f / 60);
    auto l = dynamic.Readback();
    auto r = compute.Readback();
    UINT center = (left.n / 2) * left.n + left.n / 2;
    if (l[center].x < .79f || r[center].x < 4.99f)
        throw std::runtime_error("Pointer disturbance missing");
    out << "PASS 3 layouts, 3 display modes, pause and pointer injection\n";
    for (int q : {0, 2, 1})
    {
        quality = q;
        Reset();
        Advance(1.f / 60);
        out << "PASS resize " << dynamic.Size() << "\n";
    }
    m_ClientWidth = 1024;
    m_ClientHeight = 768;
    OnResize();
    ...
    out << "PASS rendered comparison screenshot and 240 frames\n";
    return 0;
}
