...
void WaterSimulation::Init(ID3D11Device *d, ID3D11DeviceContext *c, bool useCompute)
{
    device = d;
    context = c;
    compute = useCompute;
    ...
    b = Compile(L"Shaders/Compute_CS.hlsl", "CS", "cs_5_0");
...
void WaterSimulation::Reset(UINT n)
{
    ...
    collisionIndex = 0;
    for (auto &t : height)
        t = MakeTexture(compute ? DXGI_FORMAT_R32_FLOAT : DXGI_FORMAT_R8G8B8A8_UNORM,
                        compute ? D3D11_BIND_UNORDERED_ACCESS : D3D11_BIND_RENDER_TARGET);
...
std::vector<XMFLOAT2> WaterSimulation::Readback()
{
    ...
    for (UINT y = 0; y < size; y++)
        for (UINT x = 0; x < size; x++)
        {
            ...
            data[y * size + x] =
                compute ? XMFLOAT2(((float *)row)[x], 0) : XMFLOAT2(row[x * 4] / 255.f, row[x * 4 + 1] / 255.f);
        }
