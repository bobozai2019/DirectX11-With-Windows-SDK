...
GameApp::GameApp(HINSTANCE instance, bool test)
    : D3DApp(instance, L"41 DynamicDraw Water", 1440, 900), validation(test)
{
}
bool GameApp::Init()
{
...
void GameApp::Advance(float dt)
{
    ...
    while (accumulator >= 1.f / 60)
    {
        ...
        time += 1.f / 60;
        params.drop = {0, 0, 0, 0};
        params.a = powf(waveSpeed * params.n / 60.f, 2);
        params.land = landEnabled;
        float angle = time * .55f;
        float bx = .64f + .19f * cosf(angle), by = .49f + .23f * sinf(angle);
        float heading = atan2f(.23f * cosf(angle), -.19f * sinf(angle));
        params.boat = {bx, by, heading, boatEnabled ? .8f : 0};
        if (pending.x >= 0)
        {
            params.drop = {pending.x * params.n, pending.y * params.n, .8f, 0};
            pending = {-1, -1};
        }
        water.Step(params);
    }
...
void GameApp::UpdateScene(float dt)
{
    ...
    ImGui::Text("41 DYNAMICDRAW WATER");
    ...
    ImGui::Combo("Display", &display, "Shaded water\0" "Height map\0" "Hull mask\0");
    ...
    ImGui::SameLine();
    ImGui::SetNextItemWidth(150);
    ImGui::SliderFloat("Wake strength", &params.amplitude, .05f, 1);
    ImGui::SameLine();
...
void GameApp::RenderWater(const D3D11_VIEWPORT &vp)
{
    ...
    s.info = {0.f, float(water.Size()), float(display), heightScale};
    s.boat = params.boat;
    ...
    if (display == 0)
    {
        ...
        if (boatEnabled)
            draw({(params.boat.x - .5f) * 22, .18f, (params.boat.y - .5f) * 22, 3});
    }
}
...
void GameApp::NumericalTests(std::ofstream &out)
{
    ...
    hullTest.Init(m_pd3dDevice.Get(), m_pd3dImmediateContext.Get());
    hullTest.Reset(128);
    SimParams hp;
    hp.n = 128;
    hp.land = 0;
...
int GameApp::Validate()
{
    ...
    }
    {
        ...
        out << "Live " << "DynamicDraw" << ": active=" << active << " Rmax=" << maxPos
            << " Gmax=" << maxNeg << "\n";
        if (active < 20 || maxPos <= 0 || maxNeg <= 0)
