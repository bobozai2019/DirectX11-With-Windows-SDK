...
if (-not $BuildDirectory) { $BuildDirectory = Join-Path $repoRoot 'build-dynamicdraw-water' }
...
& $cmakeExe --build $BuildDirectory --config $Configuration --target 41_DynamicDraw_Water --parallel 4
if ($LASTEXITCODE -ne 0) { throw 'Build failed.' }
$executable = Join-Path $BuildDirectory "$Configuration\41 DynamicDraw Water.exe"
