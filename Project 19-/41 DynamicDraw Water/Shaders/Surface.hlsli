cbuffer Scene : register(b0) {
    ...
    float4 Eye;
    float4 Info; // reserved, texture resolution, display mode, displacement amplitude
...
float H(float2 uv) { float2 rg=Height.SampleLevel(LinearClamp,uv,0).rg; return rg.x-rg.y; }
float3 Sky(float3 r) {
