...
if (-not $BuildDirectory) { $BuildDirectory = Join-Path $repoRoot 'build-compute-water' }
...
& $cmakeExe --build $BuildDirectory --config $Configuration --target 42_Compute_Water --parallel 4
if ($LASTEXITCODE -ne 0) { throw 'Build failed.' }
$executable = Join-Path $BuildDirectory "$Configuration\42 Compute Water.exe"
